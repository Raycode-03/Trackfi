...
export async function GET(req: NextRequest) {
  ...
  if (!success) {
    ...
    try {
      ...
      console.error("⚠️  Redis GET error:", redisErr);
    }
        ...
        if (cachedMarkets) {
          ...
          for (const holding of holdingsCoins) {
            ...
            const change =
              ...
              (market.price_change_percentage_24h / 100);
            totalValue += value;
            totalChange += change;

            if (
              market.price_change_percentage_24h > topPerformer.changePercent
            ) {
              topPerformer = {
    ...
    console.error("❌ Error in GET /api/watchlist/stats:", err);
    return NextResponse.json(
